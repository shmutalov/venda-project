  ...
  Drupal.behaviors.exportCSV = {
    attach:function (context, settings) {
      ...
      $('.warehouse-export-csv').on('typeahead:select', '.search-md-input', function(ev, suggestion) {
        ...
        $(this).parent().parent().next().find('.product-barcode').attr('disabled', true).removeAttr('readonly');
        ...
        .done(function(data, status, responseText) { 
          ...
          for (var n in data) {
            ...
            '<div class="col-sm-3 small-col-padding pr-amount"><input type="text" name="products['+suggestion.nid+'][batches]['+n+'][pr_amount]" value="'+data[n].pr_amount+'" data-val="'+data[n].pr_amount+'" placeholder="Количество" class="form-control form-text input-sm decimal"/></div>' + 
      ...
      $('.warehouse-export-csv').on('click', '.remove-row', function(e) {
        ...
        if ($('.warehouse-export-csv tbody tr').length>1) {
          ...
          $(this).parent().parent().find('.product-parts').empty().append('<div class="row small-col-padding">'+
            '<div class="col-sm-3 small-col-padding pr-amount"><input type="text" placeholder="Количество" class="form-control form-text input-sm decimal"/></div>' + 
