  ...
  Drupal.behaviors.quickAdd = {
    attach:function (context, settings) {
      ...
      $('.warehouse-export-csv').on('typeahead:select', '.search-md-input', function(ev, suggestion) {
        ...
        $(this).parent().parent().next().find('.product-barcode').attr('name', 'products[' + suggestion.nid + '][barcode]').val(suggestion.barcode).removeAttr('disabled').attr('readonly','readonly');
        ...
        $('.warehouse-export-csv tbody tr:last input').val('').removeClass('error');
        $('.warehouse-export-csv tbody tr:last input').each(function() {
          $(this).attr('name', $(this).attr('name').replace(suggestion.nid, 'new')).removeAttr('readonly');
        });
        
        $(this).parent().parent().next().find('.product-barcode').removeAttr('readonly');
        ...
        .done(function(data, status, responseText) { 
          ...
          for (var n in data) {
            ...
            '<div class="col-sm-3 small-col-padding pr-amount"><input type="text" name="products['+suggestion.nid+'][batches]['+n+'][pr_amount]" value="'+data[n].pr_amount+'" data-val="'+data[n].pr_amount+'" placeholder="Количество" class="form-control form-text input-sm"/></div>' + 
			'<div class="col-sm-3 small-col-padding opt-price"><input type="text" name="products['+suggestion.nid+'][batches]['+n+'][opt_price]" value="'+data[n].opt_price+'" placeholder="Опт. цена" class="form-control form-text input-sm integer"/></div>' + 
			'<div class="col-sm-3 small-col-padding roz-price"><input type="text" name="products['+suggestion.nid+'][batches]['+n+'][roz_price]" value="'+data[n].roz_price+'" placeholder="Роз. цена" class="form-control form-text input-sm integer"/></div>' + 
            '<div class="col-sm-3 small-col-padding date-expire"><input type="text" name="products['+suggestion.nid+'][batches]['+n+'][date_expire]" value="'+data[n].date_expire+'" placeholder="Срок годности" readonly="readonly" class="form-control form-text input-sm"/></div></div>');
            
          }
          
           $('.warehouse-export-csv tr[data-nid='+ suggestion.nid +'] .product-parts').append('<div class="row small-col-padding">'+
            '<div class="col-sm-3 small-col-padding pr-amount"><input type="text" name="products['+suggestion.nid+'][batches_new][new][pr_amount]" value="" placeholder="Количество" class="form-control form-text input-sm"/></div>' + 
			'<div class="col-sm-3 small-col-padding opt-price"><input type="text" name="products['+suggestion.nid+'][batches][new][opt_price]" value="" placeholder="Опт. цена" class="form-control form-text input-sm integer"/></div>' + 
			'<div class="col-sm-3 small-col-padding roz-price"><input type="text" name="products['+suggestion.nid+'][batches][new][roz_price]" value="" placeholder="Роз. цена" class="form-control form-text input-sm integer"/></div>' + 
            '<div class="col-sm-3 small-col-padding date-expire"><input type="text" name="products['+suggestion.nid+'][batches][new][date_expire]" value="" placeholder="Срок годности" class="form-control form-text input-sm"/></div></div>');
  
           $('.warehouse-export-csv tr[data-nid='+ suggestion.nid +'] .product-parts .pr-amount > input').focus();
        });
      });
      ...
      $('.warehouse-export-csv').on('click', '.remove-row', function(e) {
        ...
        if ($('.warehouse-export-csv tbody tr').length>1) {
          ...
          $(this).parent().parent().find('.product-parts').empty().append('<div class="row small-col-padding">'+
            ...
            '<div class="col-sm-3 small-col-padding date-expire"><input type="text" placeholder="Срок годности" readonly="readonly" class="form-control form-text input-sm"/></div></div>');
        }
        
      });
      
      $('#customizer-warehouse-export-form').on("submit",function(e){
      ...
      $('.warehouse-export-csv-actions .form-submit').on('click', function(e){
        $('#customizer-warehouse-export-form').removeClass('stop-on-submit').submit();
      });
