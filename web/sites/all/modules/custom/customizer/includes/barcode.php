...
class barcode {
	...
	public static function code39($text, $name, $title = false, $height = 50, $widthScale = 1) {
		if (!preg_match('/^[A-Z0-9-. $+\/%]+$/i', $text)) {
			throw new Exception('Invalid text input.');
		}

		$text_ = strtoupper($text); // *UPPERCASE TEXT*
		...
		foreach (str_split($colors) as $i => $color) {
			if ($color == 'b') {
				...
				if($title) {
					imageFilledRectangle($barcode, $widthScale * $i, 15, $widthScale * ($i+1) -1 , $height-13, $black);
				} else {
		...
		//imageString($barcode, 2, 5, 0, $title, $black);

		imageString($barcode, 2, $textcenter, $height-13, $text_, $black);
		imagePNG($barcode, dirname(__FILE__) . "/" . $name . ".png");
	} // function code39
} // class barcode
